...
export default function WaitlistModal({
  ...
  useModalLock(wait)
  const { message, validateEmail, handleError, handleSuccess, clearMessage } =
    useErrorHandler()
  ...
  const createLeadMutation = useCreateLead({
    mutation: {
      onSuccess: (data) => {
        ...
        handleSuccess("Thanks for joining! We'll be in touch soon.")
        setIsSubmitted(true)
        setEmail('')
        setPainPoint('')
        setFirmSize('')
  ...
  return (
    ...
    >
      <div
        ...
        className="rounded-2xl max-w-3xl p-4 md:p-8 mx-auto shadow-xl border border-white/25 max-h-[90vh] md:max-h-[95vh] overflow-y-auto w-full md:w-auto relative"
      >
        {!message ? (
          <Image
            src="/logo-white.png"
          ...
          <div
            className={`top-4 left-4 right-4 z-10 text-sm text-center p-3 rounded-lg ${message.styles}`}
          >
            <p>{message.text}</p>
          </div>
        )}
